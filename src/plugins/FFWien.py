...
class FFWien(plugin):
    ...
    def getJSONDump(self):
        ...
        wirelessInterfacesURL = self.baseJSONURL+\
                "/api/CNDB-Wireless_Interface"
        wiredInterfacesURL = self.baseJSONURL+\
                "/api/CNDB-Wired_Interface"
        ...
        try: # wrappers for get() and json() will raise exceptions on errors
            ...
            for ifu in wirelessInterfacesDict['entries'] + \
                ...
                nodeURL = self.baseJSONURL + "/api/CNDB-Node/" + str(nodePid)
    ...
    def getLastEntries(self, url, lastDate):
        """ get one entry newer than the date """
        returnLinks = []
        l = self.getAllEntries(url, lastDate)
        for l in link:
            try:
    ...
    def getOneEntryPerPeriod(self, url, period=7):
        """ get one entry per period (in days) """
        returnLinks = []
        returnDate = []
        links = self.getAllEntries(url)
        self.logger.info("Received a list with %d links" % len(links))
        for l in links:
            try:
                date = datetime.strptime(l.string, "topo-%Y-%m-%d-%H:%M.tsv.gz")
            except ValueError:
            ...
            if not returnLinks:
                ...
                if date > lastDate + timedelta(days=period):
                    returnLinks.append(url+l.string)
                    returnDate.append(date)
        return returnLinks
    ...
    def getNewTopology(self, mode=""):
        """ download the topology file from FFWien website:
            mode = monthly (rebuild a full db from scratch (one file per month)"""
        ...
        if lastDateFromDB == None or not mode:
        ...
        if not mode:
            ...
            linkList = self.getOneEntryPerPeriod(self.baseTopoURL)
    ...
    def getStats(self):
        ...
        if lastScan == None:
            ...
            self.getNewTopology(mode="monthly")
        exit()
